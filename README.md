...
Since we need to add the CosmosDB connection in the Azure Function, we also need to store the `COSMOSDB_ENDPOINT`, `COSMOSDB_KEY`, `COSMOSDB_DATABASE`, `COSMOSDB_CONTAINER` in the `local.settings.json` under the `azure-function` folder.
